...
exports.GetSearch = async (req, res) => {
    ...
    db.collection.find({condition}).exec(err, data) => {
        if(existing) {
            res.status(200).send({ message: 'success', contents: {priceFilter, mealFilter, typeFilter, sortBy} })
        }
        else{
            res.status(403).send({ message: 'error', contents: {priceFilter, mealFilter, typeFilter, sortBy} })
        }
    }
